function [s] = trapezoidal_filter(l,k,M,Y)
n = length(Y); % получаем длину вектра
d = deal(zeros(1,n)); % вводим новые вектора с длинами, равной длине массива Y
p = deal(zeros(1,n));
r = deal(zeros(1,n));
s = deal(zeros(1,n));
i = 1;
...
maxY = max(Y); % находим максимальное значение вектора Y
maxS = max(s); % находим максимальное значение вектора s
for i=1:n
s(i) = maxY * s(i) / maxS; % масштабируем каждый элемент s(i) по максимальным значениям maxV и maxS
end
end